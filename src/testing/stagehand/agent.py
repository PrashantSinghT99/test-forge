...
class StagehandAgent:
    ...
    def execute_goal(self, goal: str) -> List[Dict]:
        ...
        for step in range(1, self.max_steps + 1):
            ...
            print(f"[Stagehand Agent] Decided action: {action.get('action')} | Reason: {action.get('reason')}")
    ...
    def _get_ai_action(self, goal: str, url: str, candidates: List[Dict]) -> Optional[Dict]:
    ...
    def _get_heuristic_action(self, goal: str, url: str, candidates: List[Dict]) -> Optional[Dict]:
        ...
        if "login" in goal_lower or "backpack" in goal_lower or "cart" in goal_lower:
            if has_username and has_login_btn:
                username_filled = any(h.get("action") == "fill" and h.get("selector") == "#user-name" for h in self.history)
                password_filled = any(h.get("action") == "fill" and h.get("selector") == "#password" for h in self.history)
        ...
        if "backpack" in goal_lower or "cart" in goal_lower:
            if "/inventory.html" in url:
                backpack_btn = None
                for c in candidates:
                    if "add-to-cart-sauce-labs-backpack" in c["id"] or "backpack" in c["id"] and c["tag"] == "button":
                ...
                clicked_backpack = any(h.get("action") == "click" and h.get("selector") == backpack_btn for h in self.history)
                clicked_cart_link = any(h.get("action") == "click" and h.get("selector") == ".shopping_cart_link" for h in self.history)
