...
class HealingLocator:
    ...
    def fill(self, value, **kwargs):
        ...
        return self._run_action_with_healing(
            "fill", lambda loc: loc.fill(value, **kwargs)
        )
    ...
    def type(self, text, **kwargs):
        ...
        return self._run_action_with_healing(
            "type", lambda loc: loc.type(text, **kwargs)
        )

    def press(self, key, **kwargs):
        """Executes press action with self-healing fallback handling."""
        return self._run_action_with_healing(
            "press", lambda loc: loc.press(key, **kwargs)
        )
    ...
    def _run_action_with_healing(self, action_name, action_fn, *args, **kwargs):
        ...
        try:
            ...
            if (
                len(self._healing_page.healed_attempts)
                >= self._healing_page.max_attempts
            ):
                print(
                    "[Self-Healing] Max healing attempts limit reached. Propagating error."
                )
                raise primary_exc
            ...
            try:
                ...
                print(
                    f"[Self-Healing] Captured 'before' screenshot: {before_path}",
                    file=sys.stderr,
                )
            except Exception as se:
                print(
                    f"[Self-Healing] Failed to capture before screenshot: {se}",
                    file=sys.stderr,
                )
            ...
            if not candidates:
                print(
                    "[Self-Healing] No DOM candidates extracted. Propagating error.",
                    file=sys.stderr,
                )
                raise primary_exc
            ...
            if self._healing_page.use_ai:
                try:
                    ...
                    if ai_suggestion:
                        print(
                            f"[Self-Healing] AI suggested fallback: '{ai_suggestion}'",
                            file=sys.stderr,
                        )
            ...
            for _score, c in scored_matches:
                xpath = c.get("xpath")
                css = c.get("css")
            ...
            if not fallbacks:
                print(
                    "[Self-Healing] No suitable fallback selectors found. Propagating error.",
                    file=sys.stderr,
                )
                raise primary_exc
            ...
            for fb in fallbacks:
                try:
                    print(
                        f"[Self-Healing] Attempting fallback: '{fb}'", file=sys.stderr
                    )
                    fallback_locator = self._healing_page.page.locator(fb)
                    res = action_fn(fallback_locator)
                    print(
                        f"[Self-Healing] Success! Resolved '{self._selector}' using '{fb}'",
                        file=sys.stderr,
                    )

                    after_path = screenshots_dir / f"after_heal_{ts}.png"
                    try:
                        self._healing_page.page.screenshot(
                            path=str(after_path), timeout=5000
                        )
                        print(
                            f"[Self-Healing] Captured 'after' screenshot: {after_path}",
                            file=sys.stderr,
                        )
                    except Exception as se:
                        print(
                            f"[Self-Healing] Failed to capture after screenshot: {se}",
                            file=sys.stderr,
                        )
